...
class CompletionProvider {
    constructor(mcpClient) {
        this.mandatoryTasks = new Map();
        this.pipelineSnippets = new Map();
        this.mcpClient = mcpClient;
        this.initializeCompletions();
    }
    initializeCompletions() {
        // Tareas obligatorias de seguridad (aparecen primero)
        this.createMandatoryTask('TruffleHog', 'TruffleHog@1 - Escaneo de secretos [OBLIGATORIO]', `- task: TruffleHog@1
  ...
  inputs:
    ...
    depth: 50
    maxSecrets: 0
  continueOnError: false`, '🔴 Política SEC-001: Obligatorio para todos los pipelines');
        this.createMandatoryTask('SonarQube', 'SonarQubePrepare@5 - Análisis SAST [OBLIGATORIO]', `- task: SonarQubePrepare@5
  ...
  inputs:
    ...
    cliProjectName: '\$(Build.Repository.Name)'`, '🔴 Política SEC-002: Análisis estático obligatorio');
        this.createMandatoryTask('Snyk', 'SnykSecurityScan@1 - Escaneo dependencias [OBLIGATORIO]', `- task: SnykSecurityScan@1
  ...
  inputs:
    ...
    failOnIssues: true
    monitorOnBuild: true`, '🔴 Política SEC-003: Verificación de dependencias obligatoria');
        // Snippets para stages completos
        this.createStageSnippet('Security', 'stage:security - Stage de seguridad completo', `- stage: Security
      ...
      steps:
        ...
        - task: SnykSecurityScan@1
          ...
          inputs:
            testType: 'app'
            failOnIssues: true`);
        this.createStageSnippet('Build', 'stage:build - Stage de build estándar', `- stage: Build
  ...
  jobs:
    - job: BuildJob
      ...
      steps:
        ...
        this.createConfigSnippet('cache:npm', 'Cache para NPM', `- task: Cache@2
  ...
  inputs:
    ...
    path: \$(npm_config_cache)`);
        this.createConfigSnippet('cache:nuget', 'Cache para NuGet', `- task: Cache@2
  ...
  inputs:
    ...
    restoreKeys: |
      nuget | "\$(Agent.OS)"
    path: \$(NUGET_PACKAGES)`);
        this.createConfigSnippet('keyvault', 'Azure Key Vault - Obtener secretos', `- task: AzureKeyVault@2
  ...
  inputs:
    ...
    RunAsPreJob: false`);
        // Variables seguras
        this.createVariableSnippet('var:secure', 'Variable segura desde Key Vault', `- name: \${1:variableName}
  value: '\$(\${2:SECRET_NAME})' # Desde Key Vault o variable group`);
        this.createVariableSnippet('var:group', 'Variable group', `- group: \${1:my-variable-group}`);
        // Triggers seguros
        this.createTriggerSnippet('trigger:safe', 'Trigger seguro con branches específicos', `trigger:
  branches:
    include:
      - main
      - develop
  ...
  paths:
    ...
    exclude:
      ...
      - 'docs/*'`);
        ...
        this.createServiceSnippet('service:sql', 'Configuración Azure SQL', `# Azure SQL Configuration
- name: sqlServerName
...
- name: sqlConnectionString
  value: '\$(SQL_CONNECTION_STRING)' # Desde Key Vault`);
        this.createServiceSnippet('service:redis', 'Configuración Redis Cache', `# Redis Configuration
- name: redisName
...
- name: redisConnectionString
  value: '\$(REDIS_CONNECTION_STRING)' # Desde Key Vault`);
    ...
    createMandatoryTask(key, label, insertText, documentation) {
    ...
    createStageSnippet(key, label, insertText) {
        const item = new vscode.CompletionItem(label, vscode.CompletionItemKind.Snippet);
        item.insertText = new vscode.SnippetString(insertText);
    ...
    createConfigSnippet(key, label, insertText) {
        const item = new vscode.CompletionItem(label, vscode.CompletionItemKind.Snippet);
        item.insertText = new vscode.SnippetString(insertText);
    ...
    createVariableSnippet(key, label, insertText) {
    ...
    createTriggerSnippet(key, label, insertText) {
        const item = new vscode.CompletionItem(label, vscode.CompletionItemKind.Property);
        item.insertText = new vscode.SnippetString(insertText);
        item.documentation = 'Configuración de trigger recomendada';
        this.pipelineSnippets.set(key, item);
    }
    createServiceSnippet(key, label, insertText) {
    ...
    createTaskItem(taskName, description) {
    ...
    async getProjectSpecificCompletions(document) {
        const completions = [];
        const text = document.getText();
        // Detectar tipo de proyecto
        if (text.includes('NodeTool') || text.includes('npm')) {
            // Node.js específico
            const npmAudit = new vscode.CompletionItem('npm audit - Auditoría de seguridad', vscode.CompletionItemKind.Snippet);
            npmAudit.insertText = new vscode.SnippetString(`- script: npm audit --audit-level=high\n  displayName: '🔒 Security audit'\n  continueOnError: false`);
            npmAudit.documentation = '⚠️ Recomendado para proyectos Node.js';
            npmAudit.sortText = '1npm';
            completions.push(npmAudit);
            const npmCi = new vscode.CompletionItem('npm ci - Instalación rápida', vscode.CompletionItemKind.Snippet);
        ...
        if (text.includes('DotNetCoreCLI') || text.includes('dotnet')) {
            // .NET específico
            const vulnerableCheck = new vscode.CompletionItem('Check vulnerable packages', vscode.CompletionItemKind.Snippet);
            vulnerableCheck.insertText = new vscode.SnippetString(`- task: DotNetCoreCLI@2\n  displayName: '🔒 Check vulnerable packages'\n  inputs:\n    command: 'custom'\n    custom: 'list'\n    arguments: 'package --vulnerable --include-transitive'`);
            completions.push(vulnerableCheck);
        }
        if (text.includes('Python') || text.includes('pip')) {
            // Python específico
            const safetyCheck = new vscode.CompletionItem('safety check - Python security', vscode.CompletionItemKind.Snippet);
            safetyCheck.insertText = new vscode.SnippetString(`- script: |\n    pip install safety\n    safety check --json\n  displayName: '🔒 Python safety check'`);
